...
public: // methods
    //lsh//构造函数，首先分配一个储存所有节点个数为MaxElements的空间m_pMemory
    //lsh//为每个存储空间m_pFirstFree中每个单元分配父节点*pPrev和子节点*pNext
    ...
    m_pFirstUsed( NULL )
    {
        ...
        // Clear the memory
        memset( m_pMemory, 0, sizeof( FSA_ELEMENT ) * m_MaxElements );      //notice
    ...
    USER_TYPE *alloc()
    //lsh//用于排序，m_pFirstFree最初是记录了父子点的m_pMemory
    //lsh//经此函数处理后把m_pFirstFree的第一个放在了m_pFirstUsed的第一个（注意，会造成倒序）
    //lsh//返回移动的那一个节点
    //lsh//m_pFirstFree存未用过的存储空间，m_pFirstUsed为用过的存储空间
    {
        ...
        //lsh//reinterpret_cast<USER_TYPE*>表示强制类型转换，返回的是新的节点
    }
